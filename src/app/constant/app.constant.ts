export class AppConstant {
    //  static serverUrl = 'http://34.213.109.136:5000/';
    // static serverUrl = 'http://localhost:5000/';
    static serverUrl = 'http://www.funnydayschildcare.com.au:5000/';


    static awsAccessKeyId = "AKIAIGEI7HGWYMTIV6ZA";
    static awsSecretAccessKey = "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";
    static awsRegion = 'us-east-2';
   // static awsPhotoUrl = "https://"+"s3-us-west-2.amazonaws.com/preschool-angular/";
    static awsPhotoUrl = "https://s3.us-east-2.amazonaws.com/qmbucketuploadfile/";

}
